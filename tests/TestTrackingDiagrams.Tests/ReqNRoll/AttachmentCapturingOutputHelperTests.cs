...
public class AttachmentCapturingOutputHelperTests
{
    ...
    {
        using var scope = TestTrackingDiagrams.Tracking.TestIdentityScope.Begin(
            "AttachmentTest", "AttachmentTest");

        var inner = new FakeOutputHelper();
        var wrapper = new AttachmentCapturingOutputHelper(inner);

        TestTrackingDiagrams.Tracking.StepCollector.StartStep(
            "AttachmentTest", "When", "I do something", null, null);
        wrapper.AddAttachment("/tmp/screenshot.png");
        TestTrackingDiagrams.Tracking.StepCollector.CompleteStep(
            "AttachmentTest", passed: true);

        var steps = TestTrackingDiagrams.Tracking.StepCollector.GetSteps("AttachmentTest");
        Assert.Single(steps);
