...
public static class ReportWritersConfigurationExtensions
{
    ...
    /// This overload registers a framework-specific <see cref="XUnit3ArgumentCaptureDecorator"/> that captures
    /// raw test method arguments from xUnit v3's TestContext during scenario execution, enabling rich sub-table
    /// and expandable rendering for complex objects (records, lists, nested types) passed via framework-level
    /// attributes (MemberData, ClassData, etc.) — using the same processing pipeline as the non-LightBDD
    /// xUnit3 adapter.
    /// </summary>
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static LightBddConfiguration CreateStandardReportsWithDiagrams(
        ...
        ReportConfigurationOptions options)
    {
        // Register xUnit3-specific decorator that uses XUnit3ArgumentExtractor
        configuration.ExecutionExtensionsConfiguration()
            .EnableScenarioDecorator<XUnit3ArgumentCaptureDecorator>();
