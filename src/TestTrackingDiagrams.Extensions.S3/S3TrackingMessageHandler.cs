...
public class S3TrackingMessageHandler : DelegatingHandler, ITrackingComponent
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (_options.Verbosity == S3TrackingVerbosity.Summarised && s3Op.Operation == S3Operation.Other)
        ...
        var label = S3OperationClassifier.GetDiagramLabel(s3Op, _options.Verbosity);

        var requestContent = await GetRequestContent(request, cancellationToken);
        var requestHeaders = GetFilteredHeaders(request);

        OneOf<HttpMethod, string> method = _options.Verbosity == S3TrackingVerbosity.Raw
            ? request.Method
            : label!;

        var requestUri = _options.Verbosity == S3TrackingVerbosity.Raw
            ? request.RequestUri!
            : BuildCleanUri(s3Op, _options.Verbosity);
    ...
    private async Task<string?> GetRequestContent(HttpRequestMessage request, CancellationToken ct)
    {
        ...
        if (_options.Verbosity == S3TrackingVerbosity.Summarised)
            return null;
