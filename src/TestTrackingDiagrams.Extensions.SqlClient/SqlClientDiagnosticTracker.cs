...
/// <summary>
/// Tracks SQL Server operations via the <c>"SqlClientDiagnosticListener"</c> DiagnosticSource.
/// Subscribes to <c>SqlCommand.Execute*.Start</c> and <c>SqlCommand.Execute*.Stop</c> events.
/// Zero production code changes — works automatically for all SqlClient connections.
/// </summary>
public sealed class SqlClientDiagnosticTracker : SqlDiagnosticTracker, IObserver<DiagnosticListener>, IObserver<KeyValuePair<string, object?>>
{
    ...
    public SqlClientDiagnosticTracker(SqlClientTrackingOptions options, IHttpContextAccessor? httpContextAccessor = null)
        : base(options, httpContextAccessor)
    {
    }

    public override string ComponentName => $"SqlClientDiagnosticTracker ({Options.ServiceName})";
    ...
    void IObserver<KeyValuePair<string, object?>>.OnNext(KeyValuePair<string, object?> value)
    {
        try
        {
            // SqlClient events follow the pattern: System.Data.SqlClient.WriteCommand{Before|After|Error}
            // or Microsoft.Data.SqlClient.WriteCommand{Before|After|Error}
            if (value.Key.EndsWith("WriteCommandBefore") ||
                value.Key.Contains("ExecuteReader.Start") ||
                value.Key.Contains("ExecuteNonQuery.Start") ||
                value.Key.Contains("ExecuteScalar.Start"))
            {
                HandleCommandStart(value.Value);
            }
            else if (value.Key.EndsWith("WriteCommandAfter") ||
                     value.Key.Contains("ExecuteReader.Stop") ||
                     value.Key.Contains("ExecuteNonQuery.Stop") ||
                     value.Key.Contains("ExecuteScalar.Stop"))
            {
                HandleCommandEnd(value.Value);
            }
            else if (value.Key.EndsWith("WriteCommandError") ||
                     value.Key.Contains(".Error"))
            {
    ...
    private static (string? CommandText, string? DataSource, string? Database, Guid ExecutionId, string? Parameters) ExtractCommandInfo(object payload)
    {
        ...
        // SqlClient payload has a Command property with the SqlCommand
