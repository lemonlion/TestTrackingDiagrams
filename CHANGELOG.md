...
- **`ClientNamesToServiceNames` now uses contains-based matching as a fallback** — When the exact dictionary key doesn't match the client name (e.g. Refit generates `Refit.Implementation.Generated+...+IIntelligenceAiApiClient`), the resolver now scans dictionary keys for a substring match. Exact match still takes priority. This makes `ClientNamesToServiceNames` work with Refit, source-generated clients, and other frameworks that produce complex HttpClient names. ([#65](https://github.com/lemonlion/Kronikol/issues/65))
